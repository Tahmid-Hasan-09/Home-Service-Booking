...
@php
	$date = date('d-m-y');
	$today = App\Models\Booking::where('booking_date',$date)->sum('amount');

	$month = date('F');
	$month = App\Models\Booking::where('booking_month',$month)->sum('amount');

    $year = date('Y');
	$year = App\Models\Booking::where('booking_year',$year)->sum('amount');
...
<div class="box overflow-hidden pull-up">
	<div class="box-body">							
		...
		<div>
			<p class="text-mute mt-20 mb-0 font-size-16">Pending Bookings </p>
			<h3 class="text-white mb-0 font-weight-500">{{ count($pending) }} <small class="text-danger"><i class="fa fa-caret-up"></i> Booking </small></h3>
		</div>
	</div>
</div>
</div>
...
<div class="box">
	...
	@php
$bookings = App\Models\Booking::where('status','pending')->orderBy('id','DESC')->get();
		...
		<tr>										
			<td class="pl-0 py-8">
				 <span class="text-white font-weight-600 d-block font-size-16">
					{{ Carbon\Carbon::parse($item->booking_date)->diffForHumans()  }}
				</span>
			</td>
